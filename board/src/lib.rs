...
impl Move {
    fn from_chess_move(chess_move: (String, String)) -> Move {
        let from = square_to_index(chess_move.0);
        let to = square_to_index(chess_move.1);

        Move { from: from, to }
    }

    pub fn to_chess_move(&self) -> (String, String) {
        let from = index_to_square(self.from);
        let to = index_to_square(self.to);
...
mod tests {
    ...
    mod square_to_index {
        ...
        #[should_panic]
        fn it_panics_with_unknown_square_input() {
            square_to_index(String::from("a0"));
        }

        #[test]
        fn it_converts_chess_square_to_index() {
            let index = square_to_index(String::from("a1"));
            assert_eq!(index, 81);
        }
    ...
    mod generate_square_from_string {
        ...
        fn it_panics_with_unrecognized_piece() {
            ...
            generate_square_from_string(square_char);
        }

        #[test]
        fn it_returns_square_struct() {
            let square_char = "r".chars().next().unwrap();
            let square: Square = generate_square_from_string(square_char);
            match square.piece {
    ...
    mod generate_board {
        ...
        fn it_panics_when_board_is_missing_square() {
            ...
            generate_board(board_string_missing_one);
        }
